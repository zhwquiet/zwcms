            ...
            <div class="row">
                <div class="col-xs-12">
                    <div id="master_add" style="">
                        <div class="left">
                            <div id="left_master">
                                <?php if(is_array($newlist)): $i = 0; $__LIST__ = $newlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if(($key) == "0"): ?><div class="header top">
                                            <div class="img frist_pic_<?php echo ($v["id"]); ?>"><img src="<?php echo (getattachurl($v["pic_id"])); ?>"></div>
                                            <a href="" class="frist_title_<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></a>
                                            <div class="hover">
                                                <?php if(count($newlist) > 1): ?><a href="javascript:;" class="mater-down" data-id="<?php echo ($v["id"]); ?>"><span>下移</span></a><?php endif; ?>
                                                <a href="javascript:getnewsinfo(<?php echo ($v["id"]); ?>);"><span>编辑</span></a>
                                            </div>
                                        </div>
                                    <?php else: ?>
                                        <a name="last_<?php echo ($v["id"]); ?>"></a>
                                        <div class="item">
                                            <div class="img frist_pic_<?php echo ($v["id"]); ?>"><img src="<?php echo (getattachurl($v["pic_id"])); ?>"></div>
                                            <a href="" class="frist_title_<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></a>
                                            <div class="hover">
                                                <a href="javascript:;" class="mater-up" data-id="<?php echo ($v["id"]); ?>"><span>上移</span></a>
                                                <?php if($key != (count($newlist)-1)): ?><a href="javascript:;" class="mater-down" data-id="<?php echo ($v["id"]); ?>"><span>下移</span></a><?php endif; ?>
                                                <a href="javascript:getnewsinfo(<?php echo ($v["id"]); ?>);"><span>编辑</span></a>
                                                <a href="javascript:;" class="del" data-id="<?php echo ($v["id"]); ?>"><span>删除</span></a>
                                            </div>
...
<script src="/Public/Admin/js/main.js"></script>
    ...
    <script>
        var id = '<?php echo ($newlist[0][id]); ?>';
        getnewsinfo(id);
        function getnewsinfo(id='') {
            $.ajax({
                url:"<?php echo U('Weixin/newsAdd');?>",
                data:"id="+id+"&mater_id="+'<?php echo ($_GET['id']); ?>',
                type:"GET",
        ...
        $(".del").click(function () {
            ...
            var index=layer.open({
                ...
                yes:function(){
                    $.ajax({
                        url:"<?php echo U('Weixin/materNewsDel');?>",
                        data:{id:id,mater_id:'<?php echo ($_GET['id']); ?>'},
                        type:"POST",
                        dataType:"json",
                        success:function(data){
        ...
        $(".mater-up").click(function () {
            layer.load();
            var id= $(this).attr('data-id');
            $.post("<?php echo U('Weixin/materNewsMove');?>",{id:id,mater_id:'<?php echo ($_GET['id']); ?>',type:'up'},function (data) {
                if(data.success){
                    layer.msg(data.msg, { icon: 1 });
                    setTimeout(function() {
        ...
        $(".mater-down").click(function () {
            layer.load();
            var id= $(this).attr('data-id');
            $.post("<?php echo U('Weixin/materNewsMove');?>",{id:id,mater_id:'<?php echo ($_GET['id']); ?>',type:'down'},function (data) {
                if(data.success){
                    layer.msg(data.msg, { icon: 1 });
                    setTimeout(function() {
                        window.location.reload();
        ...
        $(".choose").click(function () {
            var mater_id = '<?php echo ($_GET['id']); ?>';
            $.get("<?php echo U('Weixin/newslayer');?>",{mater_id:mater_id},function(data){
                if(data.error){
                    layer.msg(data.msg, { icon: 2 });
                }else{
                    var index=layer.open({
                        ...
                        yes:function(index, layero){
                            ...
                            $.post("<?php echo U('Weixin/newslayer');?>",{news_id:news_id,mater_id:mater_id},function (data) {
                                if(data.success){
                                    layer.msg(data.msg, { icon: 1 });
                                    setTimeout(function() {
                                        layer.close(index);
