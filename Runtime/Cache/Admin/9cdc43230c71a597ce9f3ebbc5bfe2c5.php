            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="nav_menu">
                        <ul>
                            <li class="dropdown">
                                <a href="<?php echo U('Weixin/menuAdd');?>">
                    ...
                    <div id="tablelist">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="item item<?php echo ($v["id"]); ?> child-tr child<?php echo ($v["pid"]); ?>">
                                    <td style="width: 200px;text-align: left;">
                                        <?php if(!empty($v["child"])): ?><i class="tree-icon iconfont icon-liebiaozhankai"></i>
                                            <?php else: ?>
                                    ...
                                    <td>
                                        <?php if(($v["is_show"]) == "1"): ?>是
                                            <?php else: ?>
                                            否<?php endif; ?>
                                    </td>
                                    <td class="edit">
                                        <a href="<?php echo U('Weixin/menuAdd',array('pid'=>$v['id']));?>">添加子菜单</a>
                                        <a href="<?php echo U('Weixin/menuAdd',array('id'=>$v['id']));?>">编辑</a>
                                        <a href="javascript:;" class="delete" rel="<?php echo U('Weixin/menuDel',array('id'=>$v['id']));?>">删除</a>
                                    </td>
                                </tr>
                                <?php if(!empty($v["child"])): if(is_array($v["child"])): $i = 0; $__LIST__ = $v["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cv): $mod = ($i % 2 );++$i;?><tr class="item item<?php echo ($cv["id"]); ?> child-tr child<?php echo ($cv["pid"]); ?>">
                                            <td style="width: 200px;text-align: left;">
                                                &nbsp;&nbsp;&nbsp;&nbsp;
                                            ...
                                            <td><?php echo ($cv["sort"]); ?></td>
                                            <td>
                                                <?php if(($cv["type"]) == "1"): ?>文本回复
                                                    <?php else: ?>
                                                    图文素材<?php endif; ?>
                                            </td>
                                            <td>
                                                <?php if(($cv["is_show"]) == "1"): ?>是
                                                    <?php else: ?>
                                                    否<?php endif; ?>
                                            </td>
                                            <td class="edit">
                                                <a href="<?php echo U('Weixin/menuAdd',array('id'=>$v['cid']));?>">编辑</a>
                                                <a href="javascript:;" class="delete" rel="<?php echo U('Weixin/menuDel',array('id'=>$v['cid']));?>">删除</a>
                                            </td>
...
<script src="/Public/Admin/js/main.js"></script>
    ...
    <script>
        $(".publish").click(function () {
            $.post("<?php echo U('Weixin/menuAct');?>",{},function (data) {
                if(data.success){
