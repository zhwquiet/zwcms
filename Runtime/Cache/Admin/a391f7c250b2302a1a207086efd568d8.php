            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="nav_menu">
                        <ul>
                            <li>
                                <a href="<?php echo U('Model/addField',array('id'=>$id));?>">
                                    <span class="iconfont">&#xe618;</span>添加字段
                                    <input type="hidden" id="modelid" value="<?php echo ($id); ?>"/>
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div id="tablelist">
                        ...
                        <form class="formname nice-validator n-yellow" method="post" novalidate="novalidate">
                            <table class="table table-striped table-bordered table-hover" modelid="<?php echo ($id); ?>">
                                <thead>
                                <tr class="head_nav">
                                    <th width="40">ID</th>
                                    <th width="80">排序</th>
                                ...
                                <tbody>
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr attr_id="<?php echo ($v["id"]); ?>" class="item">
                                        <td><?php echo ($v["id"]); ?></td>
                                        <td><input type="hidden" name="id" value="<?php echo ($v["id"]); ?>">
                                            <span class="msg-box n-top" style="display: none;" for="2"></span>
                                            <input type="text" name="ordnum" id="2" value="<?php echo ($v["ordnum"]); ?>" size="4" maxlength="4"
                                                   class="ordnum n-valid" aria-required="true">
                                        </td>
                                        <td class="txt_left"><?php echo ($v["fname"]); ?></td>
                                        <td><?php echo ($v["falias"]); ?></td>
                                        <td>
                                            <?php if($v["fclass"] == 1): ?>单行文本框
                                                <?php elseif($v["fclass"] == 2): ?>多行文本框
                                                <?php elseif($v["fclass"] == 3): ?>下拉列表
                                                <?php elseif($v["fclass"] == 4): ?>单选按钮
                                                <?php elseif($v["fclass"] == 5): ?>复选框
                                                <elseif/>HTML编辑器<?php endif; ?></td>
                                        <td>
                                            <?php if($v["fdatatype"] == 1): ?>文本
                                                <?php elseif($v["fdatatype"] == 2): ?>数字
                                                <?php elseif($v["fdatatype"] == 3): ?>货币
                                                <?php elseif($v["fdatatype"] == 5): ?>备注<?php endif; ?></td>
                                        <td>
                                            <?php if(($v['fismust']) == "1"): ?>是<?php else: ?> 否<?php endif; ?>
                                        </td>
                                        <td>
                                            <?php if(($v['islock']) == "1"): ?>正常<?php else: ?>锁定<?php endif; ?>
                                        </td>
                                        <td>
                                            <a href="<?php echo U('Model/editField',array('id'=>$v['id']));?>">编辑</a>
                                            <a href="javascript:void(0);" class="delete" rel="<?php echo U('Model/delField',array('id'=>$v['id']));?>">删除</a>
                                        </td>
                            ...
                            <?php if(!empty($list)): ?><tr class="table_bottom">
                                    <td colspan="9"><a class="save_btn_sort" data-url="<?php echo U('Model/reSort');?>">保存排序</a></td>
                                </tr><?php endif; ?>
                        </form>
